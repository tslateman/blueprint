...
class BlueprintGuardian(BlueprintOrchestrator):
    ...
    def _trigger_self_healing(self, blueprint_path: str, error_msg: str):
        ...
        try:
            ...
            HealerModel = BlueprintCompiler.compile_schema(
                healer_spec, model_name="HealAction"
            )
            ...
            # 3. Generate the fix
            heal_result = self.enforcer.generate(
                healer_prompt, issue_description, HealerModel
            )
            ...
            if heal_result.confidence_score > 80:
                print(
                    f"[HEAL] High confidence fix ({heal_result.confidence_score}%). Executing in Vault..."
                )
                router = ToolRouter()
                ...
                print(
                    f"[HEAL] Confidence too low ({heal_result.confidence_score}%). Manual intervention required."
                )
